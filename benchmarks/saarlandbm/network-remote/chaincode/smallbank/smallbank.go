...
func (t *SmallbankChaincode) PutState(stub shim.ChaincodeStubInterface, args []string) error {
	if len(args) != 4 { //should be [customer_id, customer_name, initial_checking_balance, initial_savings_balance]
		return fmt.Errorf(ERROR_WRONG_ARGS + " create_account")
	}
	...
	if errcheck != nil {
		return fmt.Errorf(ERROR_WRONG_ARGS + " create_account, checking balance should be integer")
	}
	saving, errsaving := strconv.Atoi(args[3])
	if errsaving != nil {
		return fmt.Errorf(ERROR_WRONG_ARGS + " create_account, saving balance should be integer")
	}
	...
	return saveAccount(stub, account)
}

// Open N accounts starting at acc[argv[1]]
func (t *SmallbankChaincode) CreateState(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	// nothing to do
	if len(args) != 3 {
		return errormsg(ERROR_WRONG_ARGS + " create_state")
	}

	nacc, err1 := strconv.Atoi(args[0])
	...
	for i := start; i < start+nacc; {
		...
		err := t.PutState(stub, []string{acc, acc, args[2], args[2]})
		if err == nil {
			i += 1
		}
	}
